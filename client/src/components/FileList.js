...
// Basic File Icon (Replace with type-specific logic if needed)
const FileIcon = () => (
...
function RenderNode({
  ...
  if (node.type === 'folder') {
    ...
    return (
      <div className="structure"> {/* Each folder/file gets its own structure container */}
        <div className="level-div" style={{ paddingLeft: indent }}>
    ...
    const fileStyle = { paddingLeft: indent + (level > 0 ? 32 : 0), paddingRight: (level > 0 ? 32 : 0) }; // Apply padding based on level
      ...
      <div className="structure"> {/* Each file gets its own structure container */}
        <div className={`level-div ${fileLevelClass}`} style={fileStyle}>
           <div className="folder-icon-name-div" style={{ flexGrow: 1 }}> {/* Use same class for consistency */}
              <FileIcon /> {/* Add specific file type icon logic here later */}
              <span className="file-name-text">{displayName}</span>
              <span className="file-size">({fileSize} bytes)</span>
           </div>
           {showDeleteButton && (
  ...
  return <div className="level-div" style={{ ...itemStyle, color: 'orange' }}>[Warning: Unknown Item Type]</div>;
}
