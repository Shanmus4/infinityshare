  ...
  if (data.type === 'meta') {
    ...
    if (!streamControllers.has(data.fileId)) {
      ...
      const headers = new Headers({
        ...
        'Content-Length': data.meta.size ? data.meta.size.toString() : undefined
      });
      ...
      caches.open('streams').then(cache => {
        const url = new URL(`/download/${data.fileId}`, self.location.origin).href;
  ...
  // Legacy compatibility for your existing code
    ...
    if (data.done) {
      ...
      if (!streamControllers.has(data.fileId)) {
        const { readable, writable } = new TransformStream();
        const controller = writable.getWriter();
        streamControllers.set(data.fileId, { controller, readable, meta: data.meta });
