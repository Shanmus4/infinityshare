...
export function useZipDownload({
  ...
  const [currentOperationDownloadedBytes, setCurrentOperationDownloadedBytes] = useState(0); // Bytes downloaded for current op
  ...
  const resetZipState = () => {
    ...
    setCurrentOperationDownloadedBytes(0); // Reset downloaded bytes
    fileData.current = {};
      ...
      pc.onicecandidateerror = (event) => {
        ...
        if (event.errorCode) {
          ...
          if (event.errorCode !== 701) {
            // 701 is often ignorable (e.g. TURN server not reachable)
            setError(`ICE candidate error: ${event.errorCode}`); // Use the hook's setError
          } else {
            console.warn(
              `[useZipDownload] ICE candidate error 701 (ignorable) for ${pcId}:`,
              event.errorText
            );
          }
        } else {
      ...
      pc.onconnectionstatechange = () => {
        ...
        switch (newState) {
          ...
          case "failed":
            ...
            setError(
              "Connection lost. Please reload the page and ensure the sender tab remains open to try again."
            );
            console.error(
      ...
      pc.ondatachannel = (event) => {
        ...
        dc.onmessage = (e) => {
          ...
          if (typeof e.data === "string" && e.data.startsWith("EOF:")) {
            ...
            setCurrentOperationDownloadedBytes(receivedBytes); // Update state for UI
        ...
        dc.onerror = (err) => {
          ...
          console.error(
            `[useZipDownload] DataChannel error for transferId: ${transferFileId}, file: ${fileName}. Error:`,
            err
          );
          setError(
            `Error during transfer of file: ${fileName}. The "Download All" operation may be incomplete.`
          );
  ...
  // Expose the unified function and the zipping folder path, connectionStatus, total size, and downloaded bytes
  return {
    startZipProcess,
    isZipping,
    zipProgress,
    ...
    currentOperationDownloadedBytes,
  };
}
