...
function App() {
    ...
    ({ fileId, data, room }) => {
      ...
      if (data && data.sdp) {
        ...
        if (sdp.type === "offer") {
          ...
          pc.setRemoteDescription(sdp).catch((e) => console.error(`[App] Error setting remote description (answer) for ${fileId}:`, e));
        }
  ...
  const handleDownloadRequest = (fileId) => {
    ...
    setTimeout(() => { if (downloadingFiles.has(fileId)) console.warn(`[App Receiver] Single download stuck for ${fileId}`); }, 10000);
  ...
  const handleDrop = (acceptedFiles) => {
    ...
    const filesWithIds = acceptedFiles.map((f) => {
      ...
      if (processedPath && processedPath.startsWith("./")) processedPath = processedPath.substring(2);
      if (processedPath) processedPath = processedPath.replace(/^\/+|\/+$/g, "");
      const finalPath = processedPath || f.name;
      return { name: f.name, size: f.size, type: f.type, file: f, fileId: makeFileId(), path: finalPath, };
    });
  ...
  useEffect(() => {
    const downloadHandler = async ({ fileId: requestedFileId, transferFileId, mainPcId, room, name, size, type, isZipRequest, isFolderRequest, folderPath, }) => {
      if (!socket.connected) {
      ...
      if (isZipRequest || isFolderRequest) {
        if (!pcIdToUse) {
          console.error(`${isZipRequest ? "Zip" : "Folder"} request without mainPcId!`);
          return;
        }
        ...
        dc.onopen = () => {
          ...
          function sendChunk() {
            if (offset < fileObj.size) {
              ...
              const slice = fileObj.file.slice(offset, offset + Math.min(chunkSize, fileObj.size - offset));
              ...
              reader.onerror = () => { delete dataChannels.current[useTransferFileId]; };
        ...
        if (isNewPc) {
          pc.createOffer()
            ...
            .then(() => {
              if (pc.localDescription) socket.emit("signal", { room: driveCode, fileId: pcIdToUse, data: { sdp: pc.localDescription }, });
              else { setError("Sender: Failed to set local description."); cleanupWebRTCInstance(pcIdToUse); }
            })
  ...
  useEffect(() => {
    const handler = ({ filesMeta }) => setReceiverFilesMeta(Array.isArray(filesMeta) ? filesMeta : []);
  ...
  useEffect(() => {
    const handler = (event) => {
      if (event.data.type === "download-ready") window.open(event.data.url, "_blank");
    };
    navigator.serviceWorker.addEventListener("message", handler);
    return () => navigator.serviceWorker.removeEventListener("message", handler);
  }, []);
  ...
  useEffect(() => {
    const handleBeforeUnload = (event) => {
      let needsConfirmation = (step === "uploaded" && files.length > 0) || (step === "receiver" && isZipping);
      if (needsConfirmation) {
        const msg = step === "uploaded" ? "Leaving will stop sharing." : "Leaving will interrupt download/zip.";
  ...
  const handleDeleteFile = (fileId) => {
    ...
    setFiles(newFiles);
    if (driveCode) {
      const filesMeta = newFiles.map(({ name, size, type, fileId, path }) => ({ name, size, type, fileId, path, }));
      socket.emit("file-list", { room: driveCode, filesMeta });
    }
  };
  ...
  const handleDeleteFolder = (folderPath) => {
    ...
    const newFiles = filesRef.current.filter(f => !(f.path === folderPath || (f.path && f.path.startsWith(pathPrefix))));
  ...
  const handleJoinDrive = (codeToJoin) => {
    ...
    if (upperCode && upperCode.length === 4 && /^[A-Z]+$/.test(upperCode)) {
      setTimeout(() => { window.location.href = `/${upperCode}?as=receiver`; }, 500);
    } else {
  ...
  function formatEtr(seconds) {
    if (seconds === null || seconds === Infinity || seconds < 0 || isNaN(seconds)) return "--:--";
  ...
  const AppHeader = () => (
    <div className="app-header">
      ...
      <div className="header-icons-right">
        <div className="home-link header-icon-link" onClick={() => (window.location.href = "/")} title="Home">
          <svg xmlns="http://www.w3.org/2000/svg" height="48px" viewBox="0 0 24 24" width="48px" fill="#000000" className="home-icon header-icon-svg">
        ...
        <a href="https://github.com/Shanmus4/infinityshare" target="_blank" rel="noopener noreferrer" className="github-link header-icon-link">
          <svg xmlns="http://www.w3.org/2000/svg" height="48px" viewBox="0 0 24 24" width="48px" fill="#000000" className="github-icon header-icon-svg">
          ...
          <div className="content-div">
            <div className="send-receive-div">
              <div className="subhead">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.9136 13.0852C10.7224 12.8945 10.4947 12.7444 10.244 12.644L2.31399 9.46399C2.21931 9.426 2.13851 9.35996 2.08245 9.27472C2.02638 9.18949 1.99773 9.08914 2.00035 8.98715C2.00296 8.88517 2.03671 8.78642 2.09706 8.70417C2.15741 8.62191 2.24148 8.56009 2.33799 8.527L21.338 2.027C21.4266 1.99499 21.5225 1.98888 21.6144 2.00939C21.7064 2.02989 21.7906 2.07616 21.8572 2.14277C21.9238 2.20939 21.9701 2.2936 21.9906 2.38555C22.0111 2.4775 22.005 2.57339 21.973 2.662L15.473 21.662C15.4399 21.7585 15.3781 21.8426 15.2958 21.9029C15.2136 21.9633 15.1148 21.997 15.0128 21.9996C14.9108 22.0022 14.8105 21.9736 14.7253 21.9175C14.64 21.8615 14.574 21.7807 14.536 21.686L11.356 13.754C11.2552 13.5035 11.1047 13.276 10.9136 13.0852ZM10.9136 13.0852L21.854 2.147" /></svg>
                <span className="subhead-text">Send</span>
              </div>
              <DropzoneArea onDrop={handleDrop}>
                <div className="dropzone-internal-content">
                  <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48" fill="none" className="dropzone-icon"><path d="M16 38H14C9 38 6 34 6 30C6 26 9 22 14 22C16 22 17 23 17 23M32 38H34C39 38 42 34 42 30C42 26 39 22 34 22C32 22 31 23 31 23" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M14 22V20C14 15 18 10 24 10C30 10 34 15 34 20V22" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M24 40V28" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M28 30L24 26L20 30" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
        ...
        <div className="main-section">
          <div className="content-div">
            ...
            <div className="sharing-info-section">
                ...
                <div className="link-details-right-div">
                  <div className="subcontainer-padding">
                    ...
                    <button className="copy-button" onClick={() => handleCopy(receiverUrl)}>
                      <svg xmlns="http://www.w3.org/2000/svg" width="32" height="33" viewBox="0 0 32 33" fill="none"><path d="M20 27.1666H6.66667V9.83329C6.66667 9.09996 6.06667 8.49996 5.33333 8.49996C4.6 8.49996 4 9.09996 4 9.83329V27.1666C4 28.6333 5.2 29.8333 6.66667 29.8333H20C20.7333 29.8333 21.3333 29.2333 21.3333 28.5C21.3333 27.7666 20.7333 27.1666 20 27.1666ZM26.6667 21.8333V5.83329C26.6667 4.36663 25.4667 3.16663 24 3.16663H12C10.5333 3.16663 9.33333 4.36663 9.33333 5.83329V21.8333C9.33333 23.3 10.5333 24.5 12 24.5H24C25.4667 24.5 26.6667 23.3 26.6667 21.8333ZM24 21.8333H12V5.83329H24V21.8333Z" fill="black"/></svg>
                    </button>
                  </div>
              ...
              <FileList files={files} onDelete={handleDeleteFile} onDeleteFolder={handleDeleteFolder} isSender={true} />
            </div>
          </div>
          ...
          <div className="content-div">
            <div className="sharing-info-section receiver-view-container">
              <div className="receiver-top-div">
                ...
                <button className={`join-drive-button download-all-button ${isDownloadAllDisabled ? "disabled" : ""}`} onClick={() => startZipProcess()} disabled={isDownloadAllDisabled}>
                  {isZipping && !zippingFolderPath ? `Downloading... (${zipProgress.toFixed(0)}%)` : "Download All"}
                </button>
              </div>
              {(error || zipError) && (
                <div className="error-subcontainer receiver-error-subcontainer" style={{ marginBottom: "16px" }}>
                  <div className="error-field">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" className="error-icon"><path d="M12 17C12.2833 17 12.521 16.904 12.713 16.712C12.905 16.52 13.0007 16.2827 13 16C12.9993 15.7173 12.9033 15.48 12.712 15.288C12.5207 15.096 12.2833 15 12 15C11.7167 15 11.4793 15.096 11.288 15.288C11.0967 15.48 11.0007 15.7173 11 16C10.9993 16.2827 11.0953 16.5203 11.288 16.713C11.4807 16.9057 11.718 17.0013 12 17ZM11 13H13V7H11V13ZM12 22C10.6167 22 9.31667 21.7373 8.1 21.212C6.88334 20.6867 5.825 19.9743 4.925 19.075C4.025 18.1757 3.31267 17.1173 2.788 15.9C2.26333 14.6827 2.00067 13.3827 2 12C1.99933 10.6173 2.262 9.31733 2.788 8.1C3.314 6.88267 4.02633 5.82433 4.925 4.925C5.82367 4.02567 6.882 3.31333 8.1 2.788C9.318 2.26267 10.618 2 12 2C13.382 2 14.682 2.26267 15.9 2.788C17.118 3.31333 18.1763 4.02567 19.075 4.925C19.9737 5.82433 20.6863 6.88267 21.213 8.1C21.7397 9.31733 22.002 10.6173 22 12C21.998 13.3827 21.7353 14.6827 21.212 15.9C20.6887 17.1173 19.9763 18.1757 19.075 19.075C18.1737 19.9743 17.1153 20.687 15.9 21.213C14.6847 21.739 13.3847 22.0013 12 22ZM12 20C14.2333 20 16.125 19.225 17.675 17.675C19.225 16.125 20 14.2333 20 12C20 9.76667 19.225 7.875 17.675 6.325C16.125 4.775 14.2333 4 12 4C9.76667 4 7.875 4.775 6.325 6.325C4.775 7.875 4 9.76667 4 12C4 14.2333 4.775 16.125 6.325 17.675C7.875 19.225 9.76667 20 12 20Z" fill="#98282A" /></svg>
              ...
              {isZipping && (
                <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: "16px", }}>
                  <div className="info-for-user receiver-info-zipping">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="25" viewBox="0 0 24 25" fill="none" className="info-icon"><path d="M10.875 17.375C10.875 17.6734 10.9935 17.9595 11.2045 18.1705C11.4155 18.3815 11.7016 18.5 12 18.5C12.2984 18.5 12.5845 18.3815 12.7955 18.1705C13.0065 17.9595 13.125 17.6734 13.125 17.375C13.125 17.0766 13.0065 16.7905 12.7955 16.5795C12.5845 16.3685 12.2984 16.25 12 16.25C11.7016 16.25 11.4155 16.3685 11.2045 16.5795C10.9935 16.7905 10.875 17.0766 10.875 17.375ZM11.25 10.25V14.5625C11.25 14.6656 11.3344 14.75 11.4375 14.75H12.5625C12.6656 14.75 12.75 14.6656 12.75 14.5625V10.25C12.75 10.1469 12.6656 10.0625 12.5625 10.0625H11.4375C11.3344 10.0625 11.25 10.1469 11.25 10.25ZM22.3992 20.5625L12.6492 3.6875C12.5039 3.43672 12.2531 3.3125 12 3.3125C11.7469 3.3125 11.4937 3.43672 11.3508 3.6875L1.60078 20.5625C1.3125 21.0641 1.67344 21.6875 2.25 21.6875H21.75C22.3266 21.6875 22.6875 21.0641 22.3992 20.5625ZM4.03594 19.9086L12 6.12266L19.9641 19.9086H4.03594Z" fill="#6F5700" /></svg>
                    <span className="info-text">The files you requested are downloading and getting zipped. Please do not close or refresh the page while this process is active.</span>
                  </div>
                  <div className="progress-display-container">
                    <div className="progress-filename-text">
                      {zippingFolderPath ? `Downloading and Zipping: ${zippingFolderPath.split("/").pop()}.zip` : "Downloading and Zipping All Files..."}
                      {isZipping && currentOperationTotalSize > 0 && (<> {' ('}<span style={{ color: '#24A094' }}>{formatBytes((zipProgress / 100) * currentOperationTotalSize, 1)}</span>{' / '}{formatBytes(currentOperationTotalSize, 1)}{')'}</>)}
                    </div>
                    {zipConnectionStatus === "interrupted" && (<div className="progress-info-text connection-status-info">{zipError || "Connection interrupted, waiting for sender..."}</div>)}
                    {zipConnectionStatus !== "failed" && (<>
                        <div className="progress-bar-wrapper">
                          <div className="progress-bar-fill" style={{ width: `${zipProgress}%` }}></div>
                          <div className="progress-bar-text">{zipProgress.toFixed(1)}%</div>
                        </div>
                        <div className="progress-stats-container">
                          <span>Speed: <span className="stat-value">{zipConnectionStatus === "interrupted" ? "--" : formatSpeed(downloadSpeed)}</span></span>
                          <span>ETA: <span className="stat-value">{zipConnectionStatus === "interrupted" ? "--" : formatEtr(etr)}</span></span>
                        </div>
                        <div className="progress-info-text">
                          {zipConnectionStatus === "interrupted" ? "Download will attempt to resume if sender reconnects." : "Please wait, the download will start automatically when zipping is complete."}
                        </div>
                      </>
                    )}
                    {zipConnectionStatus === "failed" && zipError && (<div className="progress-info-text error-text">{zipError}</div>)}
                  </div>
                </div>
              )}
  ...
  useEffect(() => {
    ...
    if (step === "init" && pathDriveCode.length === 4 && /^[A-Z]+$/.test(pathDriveCode) && !asReceiver) {
